Matrix createLocalK(int element,mesh &m){
    ...
    float e = m.getParameter(SCONDPARAM_UNKNOW),
    a = m.getParameter(FIRSTPARAM_UNKNOW),
...
Vector createLocalb(int element,mesh &m){
    Vector b;
    float f = m.getParameter(THRDPARAM_UNKNOW),
    l = m.getNode(element+1).getX() - m.getNode(element).getX();
...
void applyNeumann(mesh &m,Vector &b){
    float e = m.getParameter(SCONDPARAM_UNKNOW), a = m.getParameter(FIRSTPARAM_UNKNOW);
